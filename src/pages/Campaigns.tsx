...
const mockCampaigns: Campaign[] = [
  {
    id: 1, name: "March Flash Sale 🔥", message: "Hi {name}! 🎉 Flash Sale Alert!\n\nGet 20% OFF all Hair Care products this weekend only!\n\nUse code: FLASH20\n\nShop now before stock runs out! 💇‍♀️",
    platforms: ["WhatsApp", "Instagram"], audience: "All Customers", status: "sent",
    recipients: 156, opened: 134, replied: 42, sentAt: "Mar 5, 2026",
  },
  {
    id: 2, name: "New Arrivals Alert", message: "Hey {name}! 👋\n\nWe just restocked our best sellers:\n\n✨ Ankara Bundle (6 yards) — ₦22,000\n✨ Whipped Shea Butter — ₦6,500\n\nOrder now before they sell out! 🛍️",
    platforms: ["WhatsApp", "Facebook"], audience: "Repeat Buyers", status: "active",
    recipients: 89, opened: 72, replied: 28, sentAt: "Mar 7, 2026",
  },
  {
    ...
    platforms: ["WhatsApp"], audience: "Inactive Customers", status: "scheduled",
    recipients: 34, opened: 0, replied: 0, scheduledAt: "Mar 10, 2026",
  },
  {
    ...
    platforms: ["WhatsApp", "Instagram", "Facebook"], audience: "All Customers", status: "sent",
    recipients: 201, opened: 178, replied: 67, sentAt: "Feb 12, 2026",
  },
];
...
const platformOptions = [
  ...
  { name: "Instagram", color: "bg-pink-100 text-pink-600 border-pink-300" },
...
export default function Campaigns() {
  ...
  const handleSend = (asDraft: boolean) => {
    ...
    const newCampaign: Campaign = {
      ...
      recipients: asDraft ? 0 : Math.floor(Math.random() * 100) + 50,
      opened: 0,
      replied: 0,
      scheduledAt: form.schedule || undefined,
      sentAt: !asDraft && !form.schedule ? "Just now" : undefined,
    };
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-foreground/40 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <motion.div
            ...
            className="bg-card rounded-xl shadow-card-hover p-6 w-full max-w-lg space-y-4 max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                  ...
                  {platformOptions.map((p) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border-2 transition-all ${
                        form.platforms.includes(p.name) ? p.color : "border-border text-muted-foreground"
                      }`}
                    >
                      {p.name}
                    </button>
                ...
                <p className="text-[10px] text-muted-foreground mt-1">Compliant with platform messaging rules</p>
              </div>
                  ...
                  {audiences.map((a) => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${
                        form.audience === a ? "gradient-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"
                      }`}
                    >
                      {a}
                    </button>
              ...
              <div>
                ...
                <Input
                  ...
                  className="mt-1"
                />
                <p className="text-[10px] text-muted-foreground mt-1">Leave empty to send immediately</p>
              </div>
            </div>

            <div className="flex gap-2 pt-2">
              <Button variant="outline" className="flex-1" onClick={() => handleSend(true)}>
                Save Draft
              </Button>
      ...
      <div className="space-y-3">
        {campaigns.map((c, i) => (
          <motion.div
            ...
            className="bg-card rounded-xl shadow-card p-4 md:p-5"
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="font-heading font-semibold">{c.name}</h3>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full capitalize ${statusStyles[c.status]}`}>
                    {c.status}
                  </span>
                </div>
