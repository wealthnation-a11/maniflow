...
export default function Customers() {
    ...
    const load = async () => {
      ...
      if (data) {
        setCustomers(data.map((c: any) => ({
          ...
          platform: c.platform, totalOrders: c.total_orders, totalSpent: Number(c.total_spent),
          lastOrder: c.last_order_at ? format(new Date(c.last_order_at), "MMM d, yyyy") : "—",
          status: c.status, preferences: c.preferences || [],
          firstSeen: c.first_seen ? format(new Date(c.first_seen), "MMM d, yyyy") : "—",
        })));
      }
      setDbLoading(false);
    };
  ...
  const hasFilters = platformFilter !== "all" || statusFilter !== "all" || search;
  const clearFilters = () => { setPlatformFilter("all"); setStatusFilter("all"); setSearch(""); };
    ...
    return (
      <div className="space-y-4 md:space-y-6">
        ...
        <div className="bg-card rounded-xl shadow-card">
          <EmptyState icon={Users} title="No customers yet" description="Customer profiles will be created automatically when people message you on your connected platforms." />
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        <select value={platformFilter} onChange={(e) => setPlatformFilter(e.target.value)} className="h-9 px-2 sm:px-3 rounded-lg border bg-card text-xs sm:text-sm flex-1 min-w-0 sm:flex-none">
          {allPlatforms.map((p) => <option key={p} value={p}>{p === "all" ? "Platform" : p.charAt(0).toUpperCase() + p.slice(1)}</option>)}
        </select>
          ...
          {filtered.length === 0 ? (
            ...
            <motion.div key={c.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="bg-card rounded-xl p-4 shadow-card space-y-3" onClick={() => setSelectedCustomer(c)}>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full capitalize ${platformColors[c.platform]}`}>{c.platform}</span>
              </div>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <motion.tr key={c.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setSelectedCustomer(c)}><Eye className="h-4 w-4" /></Button>
                    </td>
                  </motion.tr>
                ))}
              </tbody>
          ...
          <motion.div initial={{ opacity: 0, y: isMobile ? 100 : 0, scale: isMobile ? 1 : 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="bg-card rounded-t-2xl sm:rounded-xl shadow-card-hover p-5 sm:p-6 w-full sm:max-w-md space-y-4 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center"><span className="text-muted-foreground">Platform</span>
                <span className={`text-xs font-medium px-1.5 py-0.5 rounded-full capitalize ${platformColors[selectedCustomer.platform]}`}>{selectedCustomer.platform}</span>
              </div>
