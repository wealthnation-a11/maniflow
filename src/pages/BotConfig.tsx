...
import { generateAIResponse, type QARule, type NegotiationRule } from "@/store/botConfig";
  ...
  return (
    <div className="space-y-4 md:space-y-6 max-w-4xl">
      ...
      <Tabs defaultValue="qa" className="space-y-4">
        ...
        {/* ── Chat Preview/Test Tab ── */}
        <TabsContent value="preview" className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              Test your bot's Q&A, negotiation, and payment flow in real-time
            </p>
          ...
          <div className="bg-card rounded-xl shadow-card overflow-hidden flex flex-col" style={{ height: "min(450px, calc(100vh - 300px))" }}>
            ...
            <div className="px-4 py-3 border-b flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium">Your AI Bot</p>
                <p className="text-[10px] text-muted-foreground">Testing with current config</p>
              </div>
            </div>
              ...
              {previewMessages.length === 0 && (
                <div className="flex items-center justify-center h-full text-muted-foreground text-sm">
                  <div className="text-center space-y-2">
                    <TestTube2 className="h-8 w-8 mx-auto opacity-50" />
                    <p>Send a message to test your bot</p>
                    <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                      {["Hi", "How much is the Hair Cream Set?", "Can you do 12000?", "What do you sell?"].map((s) => (
                        <button
                          key={s}
                          onClick={() => { setPreviewInput(s); }}
                          className="text-xs bg-muted px-2.5 py-1 rounded-full hover:bg-primary/10 hover:text-primary transition-colors"
                        >
                          {s}
                        </button>
                      ))}
