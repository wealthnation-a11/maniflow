...
import { Plus, Pencil, Trash2, Package, X, Upload, Download } from "lucide-react";
        ...
        {products.map((p, i) => (
          <motion.div key={p.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-card rounded-xl shadow-card overflow-hidden group">
            <div className="h-40 bg-muted overflow-hidden relative">
              ...
              {p.stock <= 5 && p.stock > 0 && <span className="absolute top-2 right-2 text-[10px] font-medium px-2 py-0.5 rounded-full bg-warning/90 text-warning-foreground">Low Stock</span>}
              {p.stock === 0 && <span className="absolute top-2 right-2 text-[10px] font-medium px-2 py-0.5 rounded-full bg-destructive/90 text-destructive-foreground">Out of Stock</span>}
            </div>
            <div className="p-4">
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-heading font-semibold text-sm">{p.name}</h3>
                {p.category && <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">{p.category}</span>}
              </div>
              ...
              <div className="flex gap-2 mt-3">
                <Button variant="outline" size="sm" className="flex-1 text-xs" onClick={() => openEdit(p)}><Pencil className="h-3 w-3 mr-1" /> Edit</Button>
                <Button variant="outline" size="sm" className="text-destructive hover:bg-destructive/10" onClick={() => setDeleteId(p.id)}><Trash2 className="h-3 w-3" /></Button>
              </div>
            </div>
          </motion.div>
        ))}
